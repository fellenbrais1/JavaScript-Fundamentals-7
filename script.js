...
// Like with the previous project, we could reset all of the game's values to set it up for a new game. However, I think reloading the page is actually much simpler in this situation. In a situation where you need to stay within the same web session, such as in a session with another user, you would need to reset all the values to start up a new game, as reloading the page might not be in your best interests. We would have to remember to remove classes regarding changes to formatting here as well, such as the 'player--winner' class which changes the colours of a winning player's elements. We would also have to be careful with toggling the active player and the toggle state of the buttons hover, if we missed any of these things, we could mess up the order of operations for the game.

// We might want to double up the code that starts or initializes the game in the first place as a reset function as well. We could call this function automatically when the page is loaded, and then called again when a player chooses to start a new game by clicking the 'new game' button. This is good practice as it follows the DRY principle and it also allows us to force-restart the program in case of a fatal error etc. in its operation.
